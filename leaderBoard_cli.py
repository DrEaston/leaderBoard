...
astrQuizzes = ['1 - The Fusing of Elements Quiz', '2 - Solar Nuclear Fusion Quiz', 'Electrons and Light Quiz',
               'Where does light come from? - Quiz', 'The color of light - Quiz', 'Light and Scorpions - Quiz']
earthQuizzes = ["Convection Quiz", "Earth's Crust Quiz", "Earth's Mantle and Core Quiz", "Earth's Spheres Quiz",
                "Rainbows and Infrared Light", "Earth's Atmosphere Quiz", "Weather or Climate? - Videos - Quiz",
                "Weather and Climate Reading Quiz", "Wind and Solar Radiation Quiz"]
quantQuizzes = ['Valence Electrons / Energy Levels (CO Pages 8-11)', 'Protons Neutrons Electrons (CO Page 2)',
                'Electrons and Light Quiz', 'Where does light come from? - Quiz', 'The color of light - Quiz',
                'Light and Scorpions - Quiz', 'Rainbows and Infrared Light', 'Invisible Sunlight Quiz']
...
class scoreSheet:
    def __init__(self, filename):
        ...
        for x in range(len(text)):  # iterate through all the quiz scores to be added
            current_data = text[x].split(",")  # change current data into list
            try:    #catch error when headings are left in txt file and remove headings
                for y in range(len(current_data)):
                    current_data[y] = current_data[y].replace("\"", "")
                    # create and update Student objects
                if current_data[0] in self.studentIDs:  # if student is already added to studentList
                    self.appendList(current_data[0], current_data[13], current_data[10])
                else:  # if student is not there, create student and add result
                    self.createAppendList(current_data[1], current_data[2], current_data[0], current_data[13], current_data[10])
            except ValueError:
                print(f"data not entered: {filename} {str(x)}")
                print()
...
this_week=input("Hello, I am leaderBot. Let me help you.\nPlease enter the filename of this week's scores or 'default': ")
if this_week=="default":
     this_week="May4.csv"
     last_week="April28.csv"
else:
    last_week=input("Thank you! And for last week's scores: ")


# Import Data and Load into Course object
intSci = scoreSheet(this_week)
intSci.importNicknames()
lastWeek = scoreSheet(last_week)
